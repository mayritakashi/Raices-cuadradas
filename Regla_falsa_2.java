...
public class Regla_falsa_2 extends javax.swing.JFrame {

    // static String error;
    static boolean salir = false;
    //Mis variables------------------------------------
    //falsa
    static double xa = 0, xb = 0, xrm = 0, xr, fxa = 0, fxr = 0, count, xrn = 0, fxav, fxb, xrw, XRN = 0, XRNV = 0, ERROR = 0;
    ...
    //´para crear un objeto primero te vas dentro del antes del constructor y lo pondras en statico
    //despues lo agregas a tu panel con sus tamaños y la localizacion
    //definir botones
    //javax.swing.JButton btndefinir = new javax.swing.JButton("");
    public Regla_falsa_2() {
        initComponents();
                ...
                switch (sldGrado.getValue()) {
                    ...
                    case 6:
                        ...
                        if (comp < 0) {
                            ...
                            int s=0;
                            s=JOptionPane.showConfirmDialog(rootPane, "¿quiere seguir con las iteraciones?", "Alerta", JOptionPane.INFORMATION_MESSAGE);
                            if (s== JOptionPane.NO_OPTION) {
                                int seg=0;
                                seg=JOptionPane.showConfirmDialog(rootPane," ¿Seguro que deseas salir? vas a finalizar el programa", "Atencion", JOptionPane.YES_NO_OPTION);
                                if (seg== JOptionPane.NO_OPTION) {
                                    
                                }else{
                                   JOptionPane.showMessageDialog(rootPane, " Terminaste", "Alerta", JOptionPane.INFORMATION_MESSAGE);
                                System.exit(0); 
                                }
    ...
    public void continuar() {
        ...
        do {
            ...
            xr = (((fxa * xb) - (fxb * xa)) / (fxa - fxb));
            //System.out.println(xr);
            //System.out.println("fxa: " + fxa);
            err = (Math.abs((XRN - xa) / xr) * 100);
            System.out.println("Error: " + err);
            if (err <= ERROR) {
                //System.out.println("tu error es " + err);
                return;
    ...
    public void pruebas() {
        ...
        btnspiner.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                jTextField1KeyReleased(evt);
                String nombre = "^[+-]?[0-9]{1,9}(?:\\.[0-9]{1,2})?$";//validacion del nombre
                Pattern patron = Pattern.compile(nombre);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Regla_falsa_2.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Regla_falsa_2().setVisible(true);
            }
        });
    }
